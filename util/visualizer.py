...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, step, opt):

        ## convert tensors to numpy arrays
        visuals = self.convert_visuals_to_numpy(visuals, opt)
    ...
    def convert_visuals_to_numpy(self, visuals, opt):
        for key, t in visuals.items():
            tile = self.opt.batchSize > 8
            if 'input_label' == key:
                if opt.label_type == 'edge':
                    t = util.tensor2im_2(t)
                else:
                    t = util.tensor2label(t, self.opt.label_nc + 2, tile=tile)
                # visuals['color_label'] = t
            else:
                # t = util.tensor2im(t)
                # import pdb
                # pdb.set_trace()
    ...
    def save_images(self, webpage, visuals, image_path):
        visuals = self.convert_visuals_to_numpy(visuals, self.opt)
