...
# |imtype|: the desired type of the converted numpy array, from bicyclegan
def tensor2im_2(input_image, imtype=np.uint8):
...
def load_network(net, label, epoch, opt):
    ...
    for k, v in list(weights.items()):
        if k not in net.state_dict():
            print(k)
            del weights[k]
        elif v.shape != net.state_dict()[k].shape:
            print("mismatch:" + k, v.shape, "in model:", net.state_dict()[k].shape)
            del weights[k]
    # print(weights)
    # print(weights)
    # if label == 'G':
...
def patchify_image(img, n_crop, min_size=1 / 8, max_size=1 / 4):
    ...
    patches=torch.stack(patches,1).view(-1,c,target_h,target_w)
    return patches