...
class Pix2pixDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.label_type != 'edge':
            ...
            label_tensor = label.resize((self.opt.crop_size, self.opt.crop_size))
            label_tensor = transforms.ToTensor()(label_tensor)
            label_tensor = transforms.Normalize([0.5], [0.5])(label_tensor)
        # input image (real images)
        if index >= len(self.image_paths):
            print("index %d is out of range" % index)
            print("len of image paths is %d" % len(self.image_paths))
            print("len of label paths is %d" % len(self.label_paths))
